# /qa-analyze — QA Impact Agent v1.4
...
config/
  qa-agent-context.md         ← доменные правила риска и Seed Mode база (заполни под свой продукт)
  qa-product-knowledge.md     ← флоу продукта по модулям (скопируй из betzo-product/, опционально)
```
...
Скопируй из репо в свой локальный конфиг — агент подтянет её автоматически:
```bash
cp ../betzo-product/product-flows.md config/qa-product-knowledge.md
```
